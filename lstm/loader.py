...
def get_splited_dataloader(df, num_classes, batch_size):
    ...
    train_indices, remaining_indices = train_test_split(
        range(len(num_targets)), test_size=0.2, stratify=num_targets
    )
...
def doh_dataloader(path, num_classes, batch_size, train_ratio=0.8):
    df = pd.read_csv(path)
    # # if num_classes == 2:
    # #     df['label'] = df['bin']
    # # elif num_classes == 10:
    ...
    # test
    train_dataloader, test_dataloader, valid_dataloader = get_splited_dataloader(df, num_classes, batch_size)
    return train_dataloader, test_dataloader, valid_dataloader

if __name__ == '__main__':
    print()
    doh_dataloader('C:/Users/Administrator/Desktop/trafficdet-main/alg/Dataset/dataset_doh2.csv', 3, 32, 0.8)